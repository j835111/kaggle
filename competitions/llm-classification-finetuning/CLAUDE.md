...
**兩個修好之後重新驗證過，確認有效**：同一次 kernel 執行內、完全相同設定重跑
`train_fold()` 兩次，這次兩次都正常跑完（沒有被誤判攔停），分數分別是 1.08448、
1.08683，**差異只有 -0.00235**——比 TTA/校準實驗量到的雜訊量級（標準差 0.00288）
還小，落在合理的雜訊範圍內，不再是修好之前那種 0.01~0.035 的大幅飄動。這代表
現在同一次 kernel 執行內的 A/B 比較是可信的。

**用可信版本重測 label smoothing + group_by_length，結論翻盤**：同一次 kernel
執行內共用一個 baseline（1.08375），label smoothing 差異變成 **+0.03341**（雜訊
量級 10 幾倍，**正式採用**），group_by_length 差異 **-0.01421**（這次沒有發散，
證實原本判定「發散」是誤判，但拿掉誤判後它本身還是讓分數變差，**結論維持
放棄**，理由從「危險」改成「品質打折扣」）。label smoothing 前後三次測出完全
不同結論（放棄 → 沒差 → 正式採用），最可能的原因是 `load_best_model_at_end`
挑到的 checkpoint 不同——label smoothing 改的是損失函數本身，它的好處可能要
練到接近訓練結束才完全展現，舊邏輯挑歷史「最佳」子集分數的版本，可能剛好挑到
效果還沒發揮完全的中途進度去比較。這是一個好例子，說明**單一次量測、就算控制了
種子，也可能因為程式碼裡其他沒想到的隨機/選擇邏輯而失真**——同一個手法測三次
出現三種結論不是特例，是這類實驗本來就該預期的風險，值得每次都保留懷疑、找
反常數字的根本原因，不要急著把第一次測到的結果當定論。
